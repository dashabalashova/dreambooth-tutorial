...
Large text-to-image models can generate high-quality, diverse images from text, but they usually can’t faithfully reproduce and recontextualize a specific subject from only a few photos. 

**DreamBooth** fine-tunes a pretrained text-to-image model so a unique token (an identifier) in the prompt will refer to that specific subject. During training we show the model a small set of **instance images** of the subject together with **class images** (to preserve generality) – the model learns to associate the new token with the subject’s visual appearance. For example, if your identifier is the token *gmc*, you might train with instance prompts like *a photo of gmc cat*. After fine-tuning, using that token in a prompt (for example, *a photo of gmc cat walking in the forest* or *an oil portrait of gmc cat in the style of Johannes Vermeer*) produces photorealistic images of that exact cat in new poses, styles, or scenes.

This tutorial shows how to train a **LoRA-style DreamBooth** on **Stable Diffusion 3.5-medium** and run inference for recontextualization, art rendition, and property modification.
...
* `--rank` – dimension of the LoRA update matrices (default: `4`)
...
Recontextualization inference (prompts: `data/prompts/recontextualization.txt`):
```
...
Art rendition inference (prompts: `data/prompts/art.txt`):
```
python dreambooth-tutorial/inference.py \
  --pretrained_model_name_or_path stabilityai/stable-diffusion-3.5-medium \
  --lora dreambooth-tutorial/outputs/sd3-lora/pytorch_lora_weights.safetensors \
...
Property modification inference (prompts: `data/prompts/property.txt`):
```
python dreambooth-tutorial/inference.py \
  --pretrained_model_name_or_path stabilityai/stable-diffusion-3.5-medium \
  --lora dreambooth-tutorial/outputs/sd3-lora/pytorch_lora_weights.safetensors \
